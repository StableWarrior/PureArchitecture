    ...
    async def update_order(cls, payment: PaymentCallbackResponse) -> Order:
        async with Session(async_session()) as db:
            ...
            if payment_data["status"] == "succeeded":
                result = await db.orders.update_order(order_id=payment_data["order_id"], status="PAID")
            elif payment_data["status"] == "failed":
                result = await db.orders.update_order(order_id=payment_data["order_id"], status="CANCELLED")
